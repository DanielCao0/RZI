...
| # | 文件 | 干什么 |
|---|------|--------|
| 1 | `app/west.yml` | 把 `usp_zephyr` 和 `usp` 拉进工作区 |
...
| 6 | `app/zephyr/patches.yml` | 给 usp_zephyr 打本地补丁 |
...
初始化：`west init -l app && west update`（本仓库用 `./zephyr-docker.sh init`）。
...
```bash
./zephyr-docker.sh build      # 编译，FLASH 约 175 KB
./flash-rak4631.sh            # 烧录
```
