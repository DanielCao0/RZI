...
本仓库 west 在 Docker 里，命令前加 `./zephyr-docker.sh`，或先 `./zephyr-docker.sh shell`。
...
rzi/
  ...
  zephyr/                # Zephyr 源码，不是上面那个 app/zephyr/
  usp_zephyr/            # 被打补丁的模块
```

`app/zephyr/` **不是** Zephyr 源码。只是 west 默认用的名字：`WEST_PATCH_YAML = zephyr/patches.yml`，`WEST_PATCH_BASE = zephyr/patches`。

`patches.yml` 里每张补丁的 `path` 相对 `patches/`，不是相对工作区根：

| `path` | 磁盘上的文件 |
|--------|----------------|
| `usp_zephyr/0001-....patch` | `app/zephyr/patches/usp_zephyr/0001-....patch` |
...
python3 -c "
import hashlib
p='app/zephyr/patches/usp_zephyr/0003-short-name.patch'
...
west patch -l app/zephyr/patches.yml apply
```
...
```bash
./zephyr-docker.sh patch          # 撤掉再打上
./zephyr-docker.sh patch-list
./zephyr-docker.sh build          # 编 app 前会自动打补丁
```
...
2. 生成 unified diff（在该模块仓库根，路径才对）：
   ...
   ```bash
   git -C usp_zephyr diff > app/zephyr/patches/usp_zephyr/0003-short-name.patch
   ```

3. 用上一节的 Python 算 `sha256sum`。
4. 在 `patches.yml` 的 `patches:` 末尾加一项：至少 `path`、`sha256sum`、`module`、`author`、`email`、`date`。
5. `./zephyr-docker.sh patch`。若模块里还留着刚才的手改，先 `west patch clean`（或脚本里的 clean），否则 `git apply` 会报 already applied。
