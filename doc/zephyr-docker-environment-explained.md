...
```text
/home/daniel/rzi/                 ← west 工作区根 = git 仓库根
├── Dockerfile.zephyr             # 镜像：west + SDK 1.0.1
├── zephyr-docker.sh              # 进容器 / init / 编译
├── app/                          # 你的应用 + 清单（老板）
...
1. `Dockerfile.zephyr` 做出镜像 `rzi-zephyr:latest`（Ubuntu 24.04 + SDK **1.0.1**）。
2. 源码在 `rzi/zephyrproject/`，`.west` 听 `zephyr/west.yml`（Zephyr 当老板）。
3. `app/` 是树外应用；容器当时把 `zephyrproject` 挂到 `/workdir`，把 `app` 另挂到 `/app`。
4. `./zephyr-docker.sh build` 已能编 `rak4631/nrf52840`（Zephyr **4.4.99**）。

当时 Zephyr 提交（后来写进 `app/west.yml` 的 `revision`）：
...
## 第 1 步：写 `app/west.yml`（你是老板）

文件：[app/west.yml](./app/west.yml)
...
`.gitignore` 忽略 west 拉下来的树和构建产物：`.west/`、`zephyr/`、`modules/`、`usp_zephyr/`、`bootloader/`、`tools/`、`app/build/`、`build-*/`、`zephyrproject/`。

---

## 第 3 步：改 Docker 挂载（整仓 → `/workdir`）

[zephyr-docker.sh](./zephyr-docker.sh) 相对「Zephyr 当老板」时改了这些：
...
4. 默认编译：
   ...
   ```bash
   west build -p always -b rak4631/nrf52840 -d /workdir/app/build /workdir/app
   ```
...
```bash
./zephyr-docker.sh shell -lc 'west update usp_zephyr usp'
```
...
不要直接改 `usp_zephyr/`。兼容问题做成官方 `west patch`，登记在 `app/zephyr/patches.yml`，用法见 [doc/west-patch.md](./doc/west-patch.md)。`./zephyr-docker.sh build` 会先 `west patch clean` 再 `apply`。`west update` 会把模块重置到清单 revision，补丁要再打一次。
...
已验证：Zephyr **4.4.99**，板 `rak4631/nrf52840`，产物 `app/build/zephyr/zephyr.elf`（另有 `.hex`）。

旧终端若还开着 **改挂载之前** 的 `./zephyr-docker.sh shell`，先 `exit` 再进；那个容器仍按旧的 `zephyrproject` + `/app` 挂载。
...
```bash
cd /home/daniel/rzi
chmod +x zephyr-docker.sh          # 只需一次
./zephyr-docker.sh build-image     # 镜像没有时做一次
./zephyr-docker.sh init            # 已有 .west 会跳过
./zephyr-docker.sh build           # 编 app/，rak4631
./zephyr-docker.sh sample          # 编官方 periodical_uplink
./zephyr-docker.sh shell           # 工作目录 /workdir
```
