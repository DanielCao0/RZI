...
内部是：打 `west patch` → 容器里 `west build -b rak4631/nrf52840` → 产出 `app/build/zephyr/zephyr.hex`。
...
flowchart TB
  ...
  subgraph inputs ["你改的输入"]
    ...
    patches["app/zephyr/patches.yml"]
  end
  ...
  out["app/build/zephyr/zephyr.elf .hex"]
...
合并结果：`app/build/zephyr/zephyr.dts`。  
有这颗节点就会生成：
...
写在 `app/build/zephyr/include/generated/zephyr/autoconf.h`。
...
| 文件 | 用途 |
|------|------|
| `app/build/zephyr/zephyr.elf` | 调试、反汇编 |
| `app/build/zephyr/zephyr.hex` | `./flash-rak4631.sh` 烧录 |
| `app/build/compile_commands.json` | clangd（脚本会改成本机路径） |
...
```text
app/zephyr/patches.yml
app/zephyr/patches/usp_zephyr/0001-...
                              0002-...
                              0003-...
                              0004-sx1262-pa-compile-definitions.patch
```

`./zephyr-docker.sh build` 会先 `west patch clean` 再 `apply`，再配置 CMake。约定见 [west patch](./west-patch.md)。
...
| 全擦后 3.3 V | `app/src/board.c` | 板级 dts 仓 |
| 密钥、区域、SX1262 属性 | `app/boards/rak4631_nrf52840.overlay` | `usp_zephyr/boards/` |
| 开 USB / USP / 关树上 LoRa | `app/prj.conf` | — |
| 依赖版本 | `app/west.yml` | 容器里 `west init` |
| 上游 CMake/驱动缺陷 | `app/zephyr/patches/` | 直接改 `usp_zephyr/` |
