...
"""Upload a sketch or signed slot1 image over CDC (RSUP).

Type 0 (sketch) is handled by the Arduino loader.
Type 1 (signed slot) is the shared contract with RZI rzi_rsup.
"""
...
def upload(port: str, path: str, kind: int, touch: bool, vid: int | None, pid: int | None, wait_reboot: float) -> None:
    ...
    for do_touch in attempts:
        try:
            ...
            ser = None
    if ser is None:
        raise SystemExit(f"error: loader did not advertise RSUP-RDY ({last_err})")

    try:
        ser.write(header)
        ser.flush()
        wait_token(ser, b"RSUP-GO", 30.0)
