...
ZTEST(rzi_at_framework, test_attention_and_crlf)
{
	send_command("AT\r\n");
	zassert_equal(strcmp(output, "\r\nOK\r\n"), 0);
}

ZTEST(rzi_at_framework, test_command_name_is_case_insensitive)
{
	send_command("at+ver=?\r");
	zassert_not_null(strstr(output, "AT+VER=RZI_0.2.0_"));
}

ZTEST(rzi_at_framework, test_argument_case_is_preserved)
{
...
ZTEST(rzi_at_framework, test_size_errors_map_to_rui3_overflow)
{
	send_command_until("AT+CUSTOM=LARGE\r", "\r\nAT_TEST_PARAM_OVERFLOW\r\n");
	send_command_until("AT+CUSTOM=OVER\r", "\r\nAT_TEST_PARAM_OVERFLOW\r\n");
}

ZTEST(rzi_at_framework, test_operation_validation)
{
	send_command("AT+CUSTOM\r");
	zassert_not_null(strstr(output, "\r\nAT_ERROR\r\n"));
}

ZTEST(rzi_at_framework, test_registry_is_immutable_after_start)
{
	zassert_equal(rzi_at_register(&custom_command, 1), -RZI_ERR_DENIED);
}

ZTEST(rzi_at_framework, test_system_lock_and_sleep)
{
...
ZTEST(rzi_at_framework, test_p2p_and_radio_test_commands)
{
...
ZTEST_SUITE(rzi_at_framework, NULL, setup, NULL, NULL, NULL);
